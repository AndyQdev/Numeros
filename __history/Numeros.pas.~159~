...
Type
 Numero = class
  ...
  public
    ...
    Procedure SumaReales(var s: string);
    Function ConverTirAlitCentavos(s:string): string;
    ...
    Function EsSimetrico: boolean;
    function CountDigitos: byte;
    function NextPalabra(var i: word; str: string): string;
end;
...
Function Numero.ConverTirAlitCentavos(s:string): string;
const separador=[',','.'];
var  i,c: byte; b: boolean; base,  Val: cardinal;
begin
 SumaReales(s); b:=false; c:=0;
  for I := 1 to length(s) do begin
      if b then
         inc(c);
      if s[i] in separador then
         b:= true;
  end;
 base:= Trunc(power(10,c));
 Val:= trunc(StrToFloat(s)*base);
 result:= ConvertirLiteral(val div base)+' con '+ConvertirLiteral(val mod base)+' centavos.';
end;
...
function Numero.CountDigitos: byte;
var d: byte; r:real;
begin
 while D<valor do begin
     valor := valor div 10;
     d:= valor  mod   10;
 end;
 result:= d;
...
function Numero.EsSimetrico: boolean;
var
   aux: cardinal;
begin
 aux:= valor; InvertirNum;
 if aux = valor then begin
    result:= true;
 end else begin
    result:= false;
 end;
...
procedure  Numero.SumaReales(var s: string);
var i: word; sR: string; R:real;
begin
 i:=1; R:=0;
 while i<=length(S) do
      R:=R+strTofloat(NextPalabra(i,s));
 S:=FloatTostr(r);
end;
function Numero.NextPalabra(var i: word; str: string): string;
const separador=[';',':','/'];
var
   s: string;
begin
  while (i<= length(str)) and (str[i] in separador) do
         inc(i);
 s:='';
  while (I<= length(Str)) and (not(str[i] in separador)) do begin
    s:= s + str[i];
    inc(i);
  end;
 result:= s;
